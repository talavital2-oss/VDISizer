...
const TeraSkyLogo = () => (
  <div className="flex items-center gap-3">
    ...
    <span className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">TeraSky</span>
  </div>
);
...
const VdiArchitectCalculator = () => {
  ...
  const ProgressBar = ({ label, value, limit, isCpu, isStorage }) => {
    ...
    if (isStorage) color = "bg-indigo-500";

    return (
      <div className="mb-4">
        <div className="flex justify-between items-end mb-1">
          <span className="text-sm font-bold text-slate-700">{label}</span>
          <span className={`text-sm font-bold ${value > limit ? 'text-red-600' : 'text-slate-600'}`}>
            {value.toFixed(1)}%
          </span>
        </div>
    ...
    for (let c = 0; c < result.chassisCount; c++) {
      ...
      for (let s = 0; s < 8; s++) {
        if (bladesAllocated < result.totalBlades) {
          ...
          slots.push(
            <div key={s} className="h-full bg-slate-100 rounded-sm border border-slate-300 flex items-center justify-center">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-slate-800 flex items-center">
                <span className="mr-2 text-blue-600"><Icons.Server /></span>
                VDI Architect <span className="ml-2 text-sm bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded border border-indigo-200">v12.7</span>
              </h1>
              <p className="text-slate-500 text-sm mt-1">Precise Workload Migration & vSAN ESA Sizing</p>
            </div>
            ...
            <div className="flex space-x-3">
              <div className="bg-white p-1 rounded-lg border border-slate-200 shadow-sm flex items-center">
                  <button
                    ...
                    className={`flex items-center px-3 py-1.5 rounded-md text-xs font-bold transition-all ${configMode === 'manual' ? 'bg-blue-100 text-blue-700' : 'text-slate-500 hover:bg-slate-50'}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center px-3 py-1.5 rounded-md text-xs font-bold transition-all ${configMode === 'import' ? 'bg-blue-100 text-blue-700' : 'text-slate-500 hover:bg-slate-50'}`}
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 h-64 flex flex-col">
              <h2 className="font-bold text-slate-800 mb-3 flex items-center text-sm">
                <span className="mr-2 text-blue-600"><Icons.Cpu /></span> Target Processor
              </h2>
                ...
                {PROCESSORS.map((cpu) => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      />
                      <div>
                        <div className="font-bold text-xs text-slate-800">{cpu.name}</div>
                        <div className="text-[10px] text-slate-500">{cpu.desc}</div>
                      </div>
                    </div>
                    <div className="text-right text-xs">
                      <div className="font-mono font-bold text-slate-700">{cpu.baseFreq} GHz</div>
                      <div className="text-slate-500">{cpu.cores} Cores</div>
                    </div>
                  </label>
                ))}
              </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
               {configMode === 'manual' ? (
                 <>
                    <h2 className="font-bold text-slate-800 mb-4 text-sm flex items-center">
                      <span className="mr-2 text-slate-500"><Icons.Sliders /></span> Parameters
                    </h2>
                    <div className="space-y-5">
                        <div>
                          <div className="flex justify-between text-xs font-bold text-slate-500 uppercase mb-1">
                            <span>Total Users</span>
                            <span className="text-blue-600 text-sm">{numUsers}</span>
                          </div>
                          <input type="range" min="50" max="5000" step="50" value={numUsers} onChange={(e) => setNumUsers(Number(e.target.value))} className="w-full accent-blue-600" />
                        </div>
                        <div>
                          <div className="flex justify-between text-xs font-bold text-slate-500 uppercase mb-1">
                            <span>vCPU Per User</span>
                            <span className="text-blue-600 text-sm">{vcpuPerUser} vCPU</span>
                          </div>
                          <input type="range" min="2" max="16" step="1" value={vcpuPerUser} onChange={(e) => setVcpuPerUser(Number(e.target.value))} className="w-full accent-blue-600" />
                        </div>
                        <div>
                          <div className="flex justify-between text-xs font-bold text-slate-500 uppercase mb-1">
                        ...
                        <div>
                            <div className="flex justify-between text-xs font-bold text-slate-500 uppercase mb-1">
                            <span>CPU Ratio</span>
                            <span className="text-blue-600 text-sm">{oversubscription}:1</span>
                            </div>
                            <input type="range" min="3" max="10" step="0.5" value={oversubscription} onChange={(e) => setOversubscription(Number(e.target.value))} className="w-full accent-blue-600" />
                        </div>
                        
                        {isVsanEnabled && (
                            <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-lg">
                                <div className="flex justify-between text-xs font-bold text-indigo-700 uppercase mb-1">
                                    <span>Storage Per User</span>
                                    <span className="text-sm">{storagePerUser} GB</span>
                                </div>
                                <input type="range" min="30" max="500" step="10" value={storagePerUser} onChange={(e) => setStoragePerUser(Number(e.target.value))} className="w-full accent-indigo-600" />
                            </div>
                        )}
                    </div>
                 </>
                 ...
                 <>
                    <h2 className="font-bold text-slate-800 mb-4 text-sm flex items-center">
                      <span className="mr-2 text-indigo-500"><Icons.Import /></span> Workload Details
                    </h2>
                    <div className="space-y-4">
                      ...
                      <div className="bg-slate-50 p-2 rounded border border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-2">
                           <div>
                              <label className="text-[10px] text-slate-500 block mb-1">Total Net GHz</label>
                           ...
                           <div>
                              <label className="text-[10px] text-slate-500 block mb-1">Total Memory (GB)</label>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           <div>
                              <label className="text-[10px] text-slate-500 block mb-1">Total Cores</label>
                           ...
                           <div>
                              <label className="text-[10px] text-slate-500 block mb-1">Total Servers</label>
                      ...
                      <div className="bg-slate-50 p-2 rounded border border-slate-200">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                           <div>
                              <label className="text-[10px] text-slate-500 block mb-1">Total VMs (Users)</label>
                           ...
                           <div>
                              <label className="text-[10px] text-slate-500 block mb-1">Total vCPUs</label>
                      ...
                      {isVsanEnabled && (
                        <div className="bg-orange-50 p-2 rounded border border-orange-200">
                             <div className="text-[10px] font-bold text-orange-400 uppercase mb-2 border-b border-orange-200 pb-1">Storage Profiler</div>
                             <div className="grid grid-cols-3 gap-2">
                                <div>
                                    <label className="text-[10px] text-slate-500 block mb-1">Total IOPS</label>
                                ...
                                <div>
                                    <label className="text-[10px] text-slate-500 block mb-1">Raw TB</label>
                                ...
                                <div>
                                    <label className="text-[10px] text-slate-500 block mb-1">Used TB</label>
                      ...
                      <div className="bg-indigo-50 p-2 rounded border border-indigo-100">
                        <div className="text-[10px] font-bold text-indigo-400 uppercase mb-2 border-b border-indigo-200 pb-1">Observed Utilization</div>
                        <div className="mb-2">
                           <div className="flex justify-between text-xs text-slate-600 mb-1">
                             <span>CPU Util: <strong>{importData.avgCpuUtil}%</strong></span>
                           </div>
                           <input type="range" value={importData.avgCpuUtil} onChange={(e) => setImportData({...importData, avgCpuUtil: Number(e.target.value)})} className="w-full h-1.5 bg-indigo-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                        </div>
                        <div>
                           <div className="flex justify-between text-xs text-slate-600 mb-1">
                             <span>RAM Util: <strong>{importData.avgRamUtil}%</strong></span>
                           </div>
                           <input type="range" value={importData.avgRamUtil} onChange={(e) => setImportData({...importData, avgRamUtil: Number(e.target.value)})} className="w-full h-1.5 bg-indigo-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                        </div>
                      </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200">
                 <div className="flex items-center justify-between mb-3">
                    <h2 className="font-bold text-slate-800 text-sm flex items-center">
                        <span className="mr-2 text-indigo-600"><Icons.Database /></span> vSAN ESA (Express Storage Architecture)
                    </h2>
                    ...
                    <div className="space-y-3 animate-fadeIn">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-lg mb-4 w-full">
                            <button
                                ...
                                className={`flex-1 py-1 text-xs font-bold rounded-md transition-all ${diskConfigMode === 'auto' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}
                            >
                                Auto-Size
                            </button>
                            <button
                                onClick={() => setDiskConfigMode('manual')}
                                className={`flex-1 py-1 text-xs font-bold rounded-md transition-all ${diskConfigMode === 'manual' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}
                            >
                                Manual Count
                            </button>
                        </div>
                        ...
                        <div>
                            <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Disk Model (NVMe)</label>
                            <select 
                        ...
                        {diskConfigMode === 'manual' && (
                            <div>
                                <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">
                                    Disks Per Node: <span className="text-indigo-600">{manualDiskCount}</span>
                                </label>
                                <input 
                                    type="range" 
                                    min="2" 
                        ...
                        <div>
                            <label className="text-[10px] font-bold text-slate-500 uppercase block mb-1">Redundancy Policy</label>
                            <select 
                                value={raidPolicy.id} 
                            ...
                            <div className="text-[10px] text-slate-400 mt-1">
                                Storage Overhead: <span className="font-bold text-indigo-500">{(raidPolicy.overhead).toFixed(2)}x</span>
                            </div>
                        </div>
                    </div>
                 ) : (
          ...
          <div className="lg:col-span-5 space-y-6">
             ...
             <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                   <div className="text-slate-500 text-xs mb-1 uppercase tracking-wider">Total Chassis</div>
                   <div className="text-4xl font-bold text-indigo-600">{result.chassisCount}</div>
             ...
             <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                <div className="flex items-center mb-6 border-b pb-2">
                   <span className="text-blue-600 mr-2"><Icons.Activity /></span>
                   <h3 className="font-bold text-slate-800 text-sm">Projected Cluster Health</h3>
                </div>
                ...
                {isVsanEnabled && (
                    <div className="mt-4 pt-4 border-t border-slate-100">
                         <div className="flex justify-between items-end mb-2">
                             <div className="text-sm font-bold text-slate-700">vSAN Capacity Usage</div>
                             <div className="text-xs text-slate-500">{(result.storage.usableNeeded || 0).toFixed(1)} TB Usable</div>
                         </div>
                         <div className="w-full bg-slate-200 rounded-full h-3">
                              <div className="h-3 rounded-full bg-indigo-500" style={{ width: result.storage.isOversized ? '20%' : '70%' }}></div> 
                         </div>
                ...
                <div className="mt-4 text-xs text-slate-500 bg-slate-50 p-3 rounded border border-slate-100 flex justify-between">
                   <span><strong>{result.cpu.name}</strong> ({result.cpu.cores}C / {result.cpu.baseFreq}GHz)</span>
                   <span className="font-mono font-bold text-blue-600">{result.netGhzPerBlade.toFixed(0)} Net GHz</span>
                </div>

                {/* Constraint Notification */}
                <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded text-xs text-blue-800 flex items-start">
             ...
             {/* Rack View */}
             <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                <h3 className="font-bold text-slate-800 text-sm mb-4">Rack Visualization</h3>
                <ChassisVisual />
             </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
                ...
                {configMode === 'manual' ? (
                  ...
                  <div className="space-y-3 text-xs font-mono text-slate-300">
                    <div className="pb-2 border-b border-slate-700 mb-2">
                      <div className="text-slate-500 uppercase text-[10px] mb-1">Reverse Calculation</div>
                      <div className="flex justify-between text-slate-400">
                        <span>Used GHz:</span>
                    ...
                    {isVsanEnabled && (
                        <div className="pb-2 border-b border-slate-700 mb-2 text-indigo-300">
                             <div className="text-[10px] uppercase mb-1 text-indigo-400">Storage Profile</div>
                             <div className="flex justify-between">
                    ...
                    <div>
                       <div className="text-slate-500 uppercase text-[10px] mb-1">New Requirement</div>
                       <div className="flex justify-between">
             ...
             <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                <h3 className="font-bold text-slate-800 mb-4 flex items-center text-sm">
                   <span className="mr-2 text-blue-600"><Icons.Box /></span> Bill of Materials
                </h3>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                       <div className="bg-slate-50 p-2 rounded border border-slate-100">
                          <div className="text-[10px] text-slate-500 mb-1">Total Cores</div>
                          <div className="text-lg font-bold text-slate-800">{result.totalSystemCores}</div>
                       </div>
                       <div className="bg-slate-50 p-2 rounded border border-slate-100">
                          <div className="text-[10px] text-slate-500 mb-1">Total RAM</div>
                          <div className="text-lg font-bold text-slate-800">{result.totalSystemRamTB.toFixed(1)} <span className="text-xs font-normal">TB</span></div>
                       </div>
                    </div>
                    {isVsanEnabled && (
                        <div className="mt-2 bg-indigo-50 p-2 rounded border border-indigo-100">
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className="text-[10px] text-indigo-500 mb-1 uppercase font-bold">Raw Storage Provided</div>
                                    <div className="text-lg font-bold text-indigo-900">{(result.storage.totalRawCapacity || 0).toFixed(1)} <span className="text-xs font-normal">TB</span></div>
                                </div>
                                <div className="text-right">
                                    <div className="text-[10px] text-indigo-400">{result.storage.totalDisks} Drives Total</div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2 text-xs">
                      <li className="flex justify-between border-b border-slate-100 pb-1">
                          <span className="text-slate-600">UCS X9508 Chassis</span>
                          <span className="font-bold">{result.chassisCount}</span>
                      </li>
                      <li className="flex justify-between border-b border-slate-100 pb-1">
                          <span className="text-slate-600">UCS X215c M8 Node</span>
                          <span className="font-bold">{result.totalBlades}</span>
                      </li>
                      {isVsanEnabled && (
                           <li className="flex justify-between border-b border-slate-100 pb-1 text-indigo-600 bg-indigo-50 px-1 rounded">
                                <span>{selectedDisk.name}</span>
                                <span className="font-bold">{result.storage.totalDisks}</span>
                           </li>
                      )}
                      <li className="flex justify-between border-b border-slate-100 pb-1">
                          <span className="text-slate-600">UCS 9108 IFM</span>
                          <span className="font-bold">{result.chassisCount * 2}</span>
                      </li>
                      <li className="flex justify-between">
                          <span className="text-slate-600">2800W PSU</span>
